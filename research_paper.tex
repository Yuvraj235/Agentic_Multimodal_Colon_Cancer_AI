...
\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{paper_figures/fig06_gradcam_polyp_case.png}
  \caption{\textbf{Grad-CAM++ Visual Explanation} --- Sample 003, class
           \textit{polyps} (HyperKvasir test set). The colour scale
           transitions from blue/green (near-zero activation) through
           yellow to \textbf{orange/red} (maximum activation). The intense
           red--orange hotspot is tightly localised to the sessile polyp
           body in the lower-right quadrant and its immediate mucosal
           margin, while the surrounding healthy colonic wall and
           upper-frame bubble artefacts map to near-zero (blue/green).
           This focal, lesion-centred saliency is clinically interpretable:
           the model attends to the same anatomical structure an
           endoscopist uses to identify the lesion.}
  \label{fig:gradcam}
\end{figure}

Fig.~\ref{fig:gradcam} shows the Grad-CAM++ saliency map produced by
the Image Agent for Sample~003---a colonoscopy frame containing a
pale-pink sessile polyp with a broad mucosal base in the lower-right
quadrant, morphologically consistent with a Paris~Is/IIa lesion. The
GradCAM++ algorithm weights spatial activations from the final ResNet50
convolutional layer using second-order gradient signals~\cite{gradcam_plus_plus_2018},
producing a heatmap sensitive to both lesion presence and boundary.

The resulting overlay places a sharp orange--red hotspot precisely
co-localising with the polyp. The surrounding normal vascular mucosa,
haustral folds, and reflective bubbles in the upper frame register
green/blue (near-zero)---confirming that the classification is driven
by genuine polyp morphology rather than frame-level colour statistics
or imaging artefacts. This spatial coherence between AI saliency and
clinical lesion morphology is essential for clinician trust and is
the primary explainability output surfaced by the Image Agent in
every ColonAI inference pass.
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{External Cross-Vendor Validation (ColonAI~v2)}
\label{sec:v2}

A common failure mode of single-source endoscopy models is silent
\emph{vendor bias}---the network learns features specific to a particular
scope brand's illumination and HUD overlay rather than tissue
pathology. To quantify this we evaluated the v1 model's GradCAM++
localisation against pixel-level ground-truth masks on five external
polyp datasets: Kvasir-SEG (Olympus, $n{=}1000$), CVC-ColonDB ($n{=}380$),
CVC-300 ($n{=}60$), Kvasir-test ($n{=}100$), and ETIS-LaribPolypDB---a
Pentax scope dataset, completely unseen during training ($n{=}196$). For
each image we thresholded the GradCAM++ heatmap at the 75-th percentile
and computed binary IoU against the polyp mask.

Although v1 correctly classified 1\,700/1\,736 images as polyps
(98.0\,\%), its mean cross-vendor GradCAM IoU was 0.149---and on the
Pentax/ETIS-Larib subset collapsed to 0.069 (median 0.022), confirming
that the heatmap was landing on Olympus-specific illumination cues
rather than the lesion itself.

To remedy this we introduced a \textbf{mask-aligned attention loss}:
during training, the spatial activation of the ResNet50 target layer is
softmax-normalised over its $7{\times}7$ grid and constrained (via KL
divergence) to match the corresponding ground-truth polyp mask
downsampled to the same resolution. Combined with a multi-vendor polyp
pool of 2\,348 mask-having images (Kvasir-SEG + CVC-ClinicDB +
CVC-ColonDB + CVC-300 + ETIS-Larib + Kvasir-test), a class-balanced
\texttt{WeightedRandomSampler}, focal loss ($\gamma{=}2.0$), and KL
distillation from a frozen v1 teacher (so the staging and risk heads do
not drift), we trained ColonAI~v2 for four epochs starting from the v1
checkpoint.

\begin{table}[h]
\centering\small
\caption{Cross-vendor GradCAM IoU and Dice: v1 vs.\ v2 (mask-aware).
         All numbers are means over the full dataset.}
\label{tab:v2_iou}
\begin{tabular}{lcccccc}
\hline
\textbf{Dataset} & \textbf{Vendor} & \textbf{v1 IoU} & \textbf{v2 IoU} &
\textbf{$\Delta$} & \textbf{v1 Dice} & \textbf{v2 Dice} \\
\hline
Kvasir-SEG        & Olympus & 0.236 & \textbf{0.422} & \makebox[0pt][l]{$+79\%$} & 0.359 & 0.546 \\
ETIS-LaribPolypDB & Pentax  & 0.069 & \textbf{0.163} & \makebox[0pt][l]{$+136\%$} & 0.112 & 0.234 \\
CVC-ColonDB       & Olympus & 0.109 & \textbf{0.205} & \makebox[0pt][l]{$+88\%$}  & 0.181 & 0.301 \\
CVC-300           & Olympus & 0.118 & \textbf{0.134} & \makebox[0pt][l]{$+14\%$}  & 0.206 & 0.225 \\
Kvasir-test       & Olympus & 0.213 & \textbf{0.415} & \makebox[0pt][l]{$+95\%$}  & 0.325 & 0.533 \\
\hline
\textbf{Mean}     &         & 0.149 & \textbf{0.268} & \textbf{$+80\%$} & 0.236 & 0.368 \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:v2_iou} reports a mean cross-vendor IoU lift of
$\mathbf{+80\,\%}$, with the most clinically meaningful gain on the
Pentax/ETIS dataset ($\mathbf{+136\,\%}$, from 0.069 to 0.163). Polyp
classification accuracy on the same external datasets is essentially
preserved (1\,732/1\,736 still classified as polyps); the trade-off was
slightly lower in-distribution validation accuracy (87.0\,\% vs.\
99.5\,\%) because the class-balanced sampler is now exposing real
weaknesses in the rare UC grades that the original imbalanced split
masked.

Post-training, temperature scaling fitted by grid-search to minimise
Expected Calibration Error reduced ECE from 0.168 to \textbf{0.062}
($-63\,\%$); the optimal $T = 0.45$ indicates v2 is in fact
\emph{under}-confident on the validation distribution.

\subsection{Polyp Segmentation Head}

To provide pixel-level localisation rather than only weakly-supervised
heatmaps, we attached a lightweight three-stage U-Net-style decoder
($\approx$2.4\,M parameters) to the \emph{frozen} v2 ResNet50 backbone
and trained it on the same 2\,348 mask-supervised polyp images using
combined BCE + Dice loss for four epochs at $\textrm{lr}{=}3{\times}10^{-4}$.

\begin{table}[h]
\centering\small
\caption{Polyp segmentation IoU across the five cross-vendor datasets,
         held-out 15\,\% split (epoch~3 checkpoint).}
\label{tab:seg_iou}
\begin{tabular}{lcc}
\hline
\textbf{Dataset} & \textbf{Vendor} & \textbf{Decoder IoU} \\
\hline
ETIS-LaribPolypDB & Pentax  & \textbf{0.609} \\
CVC-ClinicDB      & Olympus & 0.609 \\
CVC-ColonDB       & Olympus & 0.616 \\
Kvasir-SEG        & Olympus & 0.618 \\
Kvasir-test       & Olympus & 0.624 \\
CVC-300           & Olympus & 0.625 \\
\hline
\textbf{Mean}     &         & \textbf{0.610} \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:seg_iou} shows that the decoder reaches a held-out
validation IoU of \textbf{0.610}---$2.3{\times}$ the v2 GradCAM IoU on
the same data, and---critically---essentially \emph{equal} performance
across Olympus and Pentax scopes ($0.609$ vs.\ $0.61\!\pm\!0.01$). The
vendor-bias gap that motivated the v2 retrain is effectively closed when
explicit pixel supervision is available. Because the encoder is frozen,
the existing classification, staging, and risk heads are entirely
unaffected; the decoder runs as an additional inference branch.

\subsection{CADe (Computer-Aided Detection) Metrics}

Treating each external image as a single-polyp detection problem and
deriving the predicted bounding box from the segmentation decoder's
thresholded output, ColonAI~v2 achieves the following CADe metrics
(Table~\ref{tab:cade}). For comparison we also report the metrics
using GradCAM++-derived bounding boxes, which are the only bbox source
available from the classification network alone.

\begin{table}[h]
\centering\small
\caption{CADe metrics: sensitivity at IoU $\geq 0.5$, False Positives
         Per Image (FPPI) at polyp probability $\geq 0.5$, and 11-point
         mAP at IoU $\geq 0.5$.}
\label{tab:cade}
\begin{tabular}{lccccc}
\hline
\textbf{Dataset} & \textbf{Source} & \textbf{sens@0.5} & \textbf{FPPI} & \textbf{mAP@0.5} \\
\hline
Kvasir-SEG   & GradCAM bbox     & 0.391 & 0.570 & 0.248 \\
             & \textbf{Seg bbox} & \textbf{0.746} & \textbf{0.215} & \textbf{0.635} \\
\hline
ETIS-Larib   & GradCAM bbox     & 0.122 & 0.704 & 0.123 \\
             & \textbf{Seg bbox} & \textbf{0.383} & \textbf{0.444} & \textbf{0.359} \\
\hline
CVC-ColonDB  & GradCAM bbox     & 0.139 & 0.750 & 0.131 \\
             & \textbf{Seg bbox} & \textbf{0.642} & \textbf{0.247} & \textbf{0.541} \\
\hline
CVC-300      & GradCAM bbox     & 0.017 & 0.983 & 0.091 \\
             & \textbf{Seg bbox} & \textbf{0.917} & \textbf{0.083} & \textbf{0.874} \\
\hline
Kvasir-test  & GradCAM bbox     & 0.400 & 0.540 & 0.325 \\
             & \textbf{Seg bbox} & \textbf{0.730} & \textbf{0.210} & \textbf{0.642} \\
\hline
\end{tabular}
\end{table}

The segmentation-derived bounding boxes more than \emph{double} the
detection sensitivity on every dataset and reduce FPPI by 60--90\,\%.
On CVC-300 the combined system achieves sens$=0.917$ and mAP$=0.874$,
which is competitive with dedicated polyp-detection networks reported
in the literature \cite{wang2018}.

\subsection{Patient-Safety Policy and Deployment Guards}

For real-world deployment a confident misprediction is worse than no
prediction. ColonAI now enforces a central safety policy
(\texttt{src/app/patient\_safety.py}) that returns one of three actions
\emph{before} any number is shown:

\begin{itemize}
  \item \textbf{Reject} if the input fails the endoscopy gate
        (pixel-statistics score $<$ 0.55) or if any upstream component
        raises an exception.
  \item \textbf{Abstain} (``Requires human review'') if confidence
        $< 0.75$, uncertainty $> 0.30$, GradCAM focus $< 0.15$, or
        multi-augmentation agent agreement $< 2/3$.
  \item \textbf{Show} only when all checks pass.
\end{itemize}

A SHA-256-keyed audit log is written for every prediction. In the live
video pipeline, a bounding box must persist across three consecutive
frames with $\textrm{IoU} \geq 0.20$ before being shown to the
operator---eliminating single-frame false positives that are common in
live colonoscopy feeds. A FastAPI service exposes \texttt{/predict},
\texttt{/health}, \texttt{/version}, and \texttt{/audit/today} endpoints
with the same policy enforced. End-to-end FPS on Apple Silicon (MPS) is
21.5\,FPS---above the 15-FPS threshold typically required for live
colonoscopy review.

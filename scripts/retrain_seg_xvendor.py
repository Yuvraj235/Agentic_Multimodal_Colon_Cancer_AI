"""Honest cross-vendor (Pentax/ETIS) segmentation number.

The original seg decoder pooled ALL datasets (incl. ETIS) and split randomly, so
ETIS was in BOTH train and val — the reported "cross-vendor IoU" was leaky.

This trains the decoder on OLYMPUS-ONLY datasets and evaluates on ALL of ETIS-Larib
(Pentax), fully HELD OUT — a genuine cross-vendor test. Saves to seg_head_xvendor.pth
(does NOT overwrite the live seg_head.pth) and writes the honest numbers.

Caveat: the frozen ResNet50 backbone came from the classifier (which saw a few ETIS
images via the polyp-aux loss); that is a negligible second-order effect — the decoder,
the actual segmenter, never sees ETIS here.
"""
...
from scripts.train_segmentation_head import SegDecoder, dice_loss, iou_metric

CKPT = "outputs/unified_multimodal_v2/checkpoints/best_model.pth"
SEG_OUT = Path("outputs/unified_multimodal_v2/seg_head_xvendor.pth")
REPORT = Path("outputs/unified_multimodal_v2/seg_xvendor_metrics.json")
...
ETIS_HELD = [
    ("ETIS-LaribPolypDB", "data/raw/test_polyp_datasets/TestDataset/ETIS-LaribPolypDB/images",
        ...
        for name, ir, mr in dataset_list:
            ...
            for f in sorted(ip.iterdir()):
                if f.suffix.lower() not in EXTS:
                    continue
                m = next((mp / (f.stem + e) for e in EXTS if (mp / (f.stem + e)).exists()), None)
                if m: self.samples.append((str(f), str(m), name)); n += 1
            print(f"    · {name}: {n}")
...
def eval_iou(dec, backbone, dl, device):
    ...
    for b in dl:
        ...
        pred = dec(backbone(img))
        p = (torch.sigmoid(pred) > 0.5).float()
        inter = (p * msk).sum(dim=(2, 3)); union = p.sum(dim=(2, 3)) + msk.sum(dim=(2, 3)) - inter
        dice_n = 2 * inter; dice_d = p.sum(dim=(2, 3)) + msk.sum(dim=(2, 3))
        for j in range(img.size(0)):
            u = float(union[j]); iou = float(inter[j] / u) if u > 1e-6 else 0.0
            d = float(dice_d[j]); dice = float(dice_n[j] / d) if d > 1e-6 else 0.0
            ious.append((iou, dice))
...
def main():
    ...
    print("\n[1/3] Olympus-only TRAIN datasets:")
    train_full = SegSet(OLYMPUS_TRAIN, augment=True)
    val_full = SegSet(OLYMPUS_TRAIN, augment=False)
    n_tr = int(0.85 * len(train_full))
    ...
    print("\n[1b] HELD-OUT ETIS (Pentax) test set:")
    etis_dl = DataLoader(SegSet(ETIS_HELD, augment=False), batch_size=16, shuffle=False, num_workers=2)
    print(f"  Olympus train {len(tr_idx)} · Olympus val {len(va_idx)} · ETIS held-out {len(etis_dl.dataset)}")

    print("\n[2/3] Training decoder (Olympus only) …")
    ...
    for ep in range(4):
        ...
        for step, b in enumerate(tr_dl):
            ...
            if step % 30 == 0:
                print(f"  ep{ep+1}/4 step{step:3d}/{len(tr_dl)} loss={loss.item():.4f}")
        olymp = eval_iou(dec, backbone, va_dl, device)
        print(f"  ✓ ep{ep+1} Olympus-val IoU={olymp['mean_iou']:.3f}")
        if olymp["mean_iou"] > best_olymp:
            best_olymp = olymp["mean_iou"]
            torch.save({"decoder_state": dec.state_dict(), "olympus_val_iou": best_olymp, "epoch": ep+1}, SEG_OUT)

    print("\n[3/3] HONEST held-out cross-vendor eval on ETIS (Pentax) …")
    dec.load_state_dict(torch.load(SEG_OUT)["decoder_state"])
    etis = eval_iou(dec, backbone, etis_dl, device)
    olymp = eval_iou(dec, backbone, va_dl, device)
    print(f"\n  >>> ETIS (Pentax) HELD-OUT:  IoU={etis['mean_iou']:.3f}  Dice={etis['mean_dice']:.3f}  "
          f"sens@IoU0.5={etis['sens_at_iou0.5']:.3f}  (n={etis['n']})")
    print(f"      Olympus val (same-vendor): IoU={olymp['mean_iou']:.3f}")
    REPORT.write_text(json.dumps({
        "method": "decoder trained OLYMPUS-ONLY; ETIS-Larib (Pentax) fully HELD OUT",
        "etis_pentax_heldout": etis, "olympus_val_samevendor": olymp,
        "prev_leaky_etis_iou": 0.609, "decoder": str(SEG_OUT),
    }, indent=2))
    print(f"\nSaved decoder → {SEG_OUT}\n      metrics → {REPORT}")

...
### 🔬 Full validation scorecard (signed, tiered by evidence)

Every capability, graded by **how trustworthy its evidence is** — **A** = truly external (data the model never saw), **B** = honest held-out split but same population, **C** = flagged optimistic / weak baseline. Generated by [`scripts/validate_clinical.py`](scripts/validate_clinical.py), which re-runs the cold ETIS (Pentax) test **live with 95% confidence intervals** → [`clinical_validation_report.md`](outputs/unified_multimodal_v2/clinical_validation_report.md).
...
| Polyp characterization (CADx) | **B** | balanced-acc 0.85 | The "benign" call always defers to histology |
| Stage from doctor's TNM | **B** | acc 0.997 | Deterministic AJCC rule — the only staging the system trusts |
| Histology tissue | **C** | macro-F1 0.996 | A tile-level split over-states this; demonstrator only |
| Stage from demographics | **C** | acc 0.53 | Kept only to show *why* we refuse to guess stage from age/sex/BMI |

> The report is **signed with the model's SHA fingerprint**, and the cross-vendor number is recomputed live each run, so it can't quietly go stale. The biggest open gap it names: true external validation of the *finding* model + a strict patient-level histology test.
